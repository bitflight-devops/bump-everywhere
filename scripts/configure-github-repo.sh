...
set_origin() {
  ...
  git remote set-url origin "${origin_url}" ||
    {
      echo "Failed to set remote URL to ${origin_url}"
      exit 1
    }
}
...
set_user() {
  ...
  git config --local user.name "${user_name}" ||
    {
      echo "Failed to set local user to ${user_name}"
      exit 1
    }
  local -r user_email="${user_name}@users.noreply.github.com"
  git config --local user.email "${user_email}" ||
    {
      echo "Failed to set local user email to ${user_email}"
      exit 1
    }
}
...
validate_parameters() {
  ...
  if utilities::is_empty_or_null "${access_token}"; then

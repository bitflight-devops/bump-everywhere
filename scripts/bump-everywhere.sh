...
clone() {
  ...
  git clone "https://x-access-token:${git_token}@github.com/${repository}.git" "${temp_directory}" ||
    {
      echo "git clone failed for ${repository}"
      exit 1
    }
  cd "${temp_directory}" ||
    {
      echo "Could not locate folder ${temp_directory}"
      exit 1
    }
  ...
  if utilities::has_value "${branch}"; then
    if [[ $(git branch --show-current) != "${branch}" ]]; then
      git switch "${branch}" || {
        echo >&2 "❌ Could not switch to specified branch: \"${branch}\"."
        exit 1
      }
    fi
  fi
...
configure_credentials() {
  ...
  bash "${SCRIPTS_DIRECTORY}/configure-github-repo.sh" \
    ...
    --token "${git_token}" ||
    {
      echo "Could not configure credentials"
      exit 1
    }
}
...
create_changelog() {
  ...
  echo "${logs}" >"${file_name}" ||
    {
      echo "Could not write logs to ${file_name}"
      exit 1
    } # printf removes /n's in start & end
  git add "${file_name}" ||
    {
...
update_npm() {
  ...
  bash "${SCRIPTS_DIRECTORY}/bump-npm-version.sh" ||
    {
      echo "Could not bump npm version"
      exit 1
    }
  git add . ||
    {
...
commit_and_push() {
  ...
  if has_uncommited_changes; then
    ...
    git commit -m "${printable_commit_message}" ||
      {
        echo "Could not commit wit the message: ${printable_commit_message}"
        exit 1
      }
    git push -u origin ||
      {
        echo "Could not git push changes"
        exit 1
      }
  fi
}
...
validate_parameters() {
  ...
  if utilities::is_empty_or_null "${repository}"; then
    echo "Repository name is not set."
    exit 1
  fi
  if utilities::is_empty_or_null "${git_user}"; then
    echo "Git user is not set."
    exit 1
  fi
  if utilities::is_empty_or_null "${git_token}"; then
    echo "Git access token is not set."
    exit 1
  fi
  if utilities::is_empty_or_null "${release_type}"; then
    echo "Release type is not set."
    exit 1
  fi
  if utilities::is_empty_or_null "${release_token}"; then
    echo "Release access token is not set."
    exit 1
  fi
  if utilities::is_empty_or_null "${commit_message}"; then
    echo "Commit message is not set."
    exit 1
  fi
  if utilities::has_value "${branch}" && ! is_valid_branch_name "${branch}"; then
    echo >&2 "❌ \"${branch}\" is not a valid branch name."
    exit 1
  fi
}

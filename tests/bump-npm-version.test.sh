...
test_json_version_bump() {
  ...
  setup_git_env "${initial_version}" "${new_version}" || {
    echo "Could not setup the test environment"
    return 1
  }
...
setup_git_env() {
  ...
  git add . || {
    echo '😢 Could not add git changes'
    return 1
  }
  git commit -m "inital commit" --quiet || {
    echo '😢 Could not do initial commit'
    return 1
  }
  git tag "${initial_version}" || {
    echo '😢 Could tag first version'
    return 1
  }
  git commit -m "next commit" --allow-empty --quiet || {
    echo '😢 Could not do next commit'
    return 1
  }
  git tag "${new_version}" || {
    echo '😢 Could tag next version'
    return 1
  }
  return 0
}
